...
```
GET /api/deeplink?short_code=abc123
```
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `short_code` | string | Yes | The short code from the deep link URL |
...
func handleDeepLink(url: URL) {
    // Extract short_code from URL: https://yourdomain.depl.link/abc123
    let shortCode = url.lastPathComponent

    // Fetch deep link data
    var request = URLRequest(url: URL(string: "https://depl.link/api/deeplink?short_code=\(shortCode)")!)
...
fun handleDeepLink(uri: Uri) {
    // Extract short_code from URL: https://yourdomain.depl.link/abc123
    val shortCode = uri.lastPathSegment

    // Fetch deep link data
    val request = Request.Builder()
        .url("https://depl.link/api/deeplink?short_code=$shortCode")
